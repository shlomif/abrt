...
static configuration_t *configuration_new(char *file_path, char *def_file_path)
{
...
static dbus_conf_node_t *dbus_conf_node_from_node(GDBusNodeInfo *node)
{
    ...
    for (size_t i = 0; i < sizeof(annots)/(sizeof(annots[0])); ++i)
    {
    ...
    char *const conf_file = xstrdup(annots[0].value);
    char *const def_conf_file = xstrdup(annots[1].value);
    ...
    GHashTable *properties = g_hash_table_new_full(g_str_hash, g_str_equal, (GDestroyNotify)NULL, (GDestroyNotify)property_free);
    for(GDBusPropertyInfo **prop_iter = node->interfaces[0]->properties; *prop_iter != NULL; ++prop_iter)
    {
         ...
         */
        parse_annotations((*prop_iter)->annotations, annots, sizeof(annots)/sizeof(annots[0]));
        ...
        if (annots[0].value != NULL && annots[1].value != NULL)
        {
            ...
            GSList *lst_item = g_slist_find_custom(configurations, (gpointer)paths, (GCompareFunc)configuration_compare_paths);

            /* If such configuration object does not exist yet, create a new one. */
            if (lst_item == NULL)
            {
                char *const prop_conf_file = xstrdup(annots[0].value);
                char *const prop_def_conf_file = xstrdup(annots[1].value);
...
static GList *load_configurators(const char *file_dir)
{
    ...
    {
        const file_obj_t *const file = (file_obj_t *)iter->data;
        const char *const fullpath = file->fullpath;
